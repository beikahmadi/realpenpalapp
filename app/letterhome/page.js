...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        let cachedLetters = JSON.parse(localStorage.getItem("lettersCache"));
        if (cachedLetters) {
        ...
        console.error("Error loading cached letters:", err);
      }
  ...
  const fetchFreshLetters = async () => {
    try {
      ...
      for (const id of letterboxIds) {
        ...
        let draft;
        try {
          draft = await fetchDraft(id, userRef, false);
        } catch (error) {
          console.error("Error fetching fresh draft:", error);
        }

        letterbox.letters = draft ? [draft] : await fetchLetterbox(id, 1);
        freshLetters.push(letterbox);
      }

      // Fetch recipients
      for await (const l of freshLetters) {
        try {
          l.recipients = await fetchRecipients(l.id);
        } catch (error) {
          console.error("Error fetching fresh recipients:", error);
        }
      }

      // Update state and cache
      setLetters(freshLetters);
      localStorage.setItem("lettersCache", JSON.stringify(freshLetters));
      console.log("Cache updated with fresh data");
    } catch (err) {
      console.error("Error fetching fresh letters:", err);
      setError("Error loading letters. Try again later.");
    }
  };
